...
class AdminController extends Controller {
    ...
    public function users() {
        ...
        if ($val = $this->request->input('val')) {
            if ($this->isDemo()) $this->defendDemo();
            if(isset($val['ghost_artist']) && $val['ghost_artist'] == 1) {
    ...
    public function tracks() {
        ...
        if ($action= $this->request->input('move_flag') == '1') {
            if ($this->isDemo()) $this->defendDemo();
            $move_to = $this->request->input('move_to_name');
            $move_track_ids = $this->request->input('move_track_ids');
            if($move_to=="") {
                return json_encode(array(
                    'message' => l("inp_username"),
                    'type' => 'error'
                ));
            }
            if($move_track_ids=="") {
                return json_encode(array(
                    'message' => l("select-track"),
                    'type' => 'error'
                ));
            }
            $ret = $this->model('admin')->moveTrack($move_to, $move_track_ids);
            if($ret<0) {
                return json_encode(array(
                    'message' => l("inp_dataerror"),
                    'type' => 'error'
                ));
            }
            return json_encode(array(
