...
class AdminModel extends Model {
    ...
    public function moveTrack($user_name, $ids) {
        $strarr = explode(",", $ids);
        $idcount = count($strarr) - 1;
        if($idcount <= 0)
            return -2;

        $query = $this->db->query("SELECT id from users WHERE username=?", $user_name);
        $bexist = $query->rowCount();
        if($bexist == 0)
            return -1;
        ...
        for($i = 0; $i <$idcount; $i++) {
            $track_id = $strarr[$i];
            $sql = "UPDATE tracks SET userid = $user_id WHERE id=$track_id";
            $query = $this->db->query("UPDATE tracks SET userid = ? WHERE id=?", $user_id, $track_id);
            $sql = "UPDATE stream SET poster=$user_id WHERE trackid=$track_id";
