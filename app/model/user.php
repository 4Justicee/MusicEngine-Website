...
class UserModel extends Model {
    ...
    function getUser($id = null, $block = true, $active = 1) {
        if ($id) {
            ...
            $queryFollowers = $this->db->query("SELECT * FROM follow WHERE follow_id=?", $user['id']);
            $followers = $queryFollowers->rowCount();
            $queryFollowing = $this->db->query("SELECT * FROM follow WHERE userid=?", $user['id']);
            $following = $queryFollowing->rowCount();
    ...
    function changePasswordByUser($user, $new) {
        $password = hash_value($new);
        $this->db->query("UPDATE users SET password=? WHERE id=? ", $password, $user['id']);
        //refresh session data now
        $this->saveData($user['id'], $password);
    }
    ...
    function addUser($val, $isAdmin = false) {
        ...
        if(!isset($ghost_artist)) $ghost_artist = 0;
        $artist = $ghost_artist? 3: ($artist ? 2 : 1);
    ...
    public function saveUser($val, $user, $isAdmin = false) {
        ...
        $this->db->query("UPDATE users SET wallet=?,currency=?,username=?,email=?,full_name=?,bio=?,avatar=?,cover=?, gender=?,country=?,city=?,website=?,facebook=?,twitter=?,youtube=?,vimeo=?,soundcloud=?,snapchat=?,
          notifyl=?,notifyc=?,notifym=?,notifyf=?,email_l=?,email_c=?,email_f=?, email_letter=?,instagram=? WHERE id=?",
                $wallet,$currency,$username, $email,$full_name,$bio,$avatar, $cover, $gender,$country,$city,$website,$facebook,$twitter,$youtube,$vimeo,$soundcloud,$snapchat,
        ...
        if (isset($val['artist'])) {
            if (config('user-signup-artist', true) or $isAdmin) {
                $artist = ($val['artist'] == 3) ? 3: (($val['artist'] == 2) ? 2 : 1);
    ...
    public function saveUserEdit($val, $user, $isAdmin = false) {
        /**
         * @var $full_name
         * @var $gender
         * @var $country
    ...
    public function getPeople($type, $term = null, $limit = 10, $offset = 0, $param = array()) {
        if ($type == 'suggestion') {
            ...
            $sql = "SELECT *, (SELECT count(id) as count FROM tracks WHERE tracks.userid = users.id) as count FROM users WHERE id NOT IN ($hideusers) AND (user_type=? OR user_type=?)  ";
            $param = array(2,3);
