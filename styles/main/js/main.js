...
function OnSelectAll() {
    var table = $("#select_all_id").parents("table");
    var childrens = table.find("input[type='checkbox'].track_sel_cls");
    var trs = table.find("tr");
    var cnt = childrens.length;
    var checklen = 0;
    var checkstr = "";
    ...
    if(checklen != cnt || checklen == 0) {
        for(i = 0; i < childrens.length; i++) {
            childrens[i].checked = true;
            id = $(trs[i+1]).find("td")[0].innerHTML;
    ...
    $("#id_sel_track").val(checkstr);
}
function select_track(id)  {
    var checkstr = $("#id_sel_track").val();
    var checke = $("#track-"+id)[0].checked;
    var repstr = id + ",";
    if(checke == true) {
        repstr = repstr + "";
        checkstr = checkstr + repstr;
    }
    else
        checkstr = checkstr.replace(repstr, "");
    $("#id_sel_track").val(checkstr);
}
